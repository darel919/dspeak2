...
const manager = await readFile(
  new URL("../server/utils/direct-messages-manager.ts", import.meta.url),
  "utf8",
);
const store = await readFile(
  new URL("../app/stores/directMessages.ts", import.meta.url),
  "utf8",
);
...
test("direct message routes authorize conversation reads, sends, and receipts", async () => {
  const [indexRoute, conversationRoute] = await Promise.all([
    readFile(
      new URL("../server/routes/api/direct-messages/index.ts", import.meta.url),
      "utf8",
    ),
    readFile(
      new URL(
        "../server/routes/api/direct-messages/[conversationId].ts",
        import.meta.url,
      ),
      "utf8",
    ),
