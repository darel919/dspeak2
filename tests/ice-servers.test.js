...
test('TURN credentials use the Coturn REST timestamp and HMAC contract', () => {
  ...
  assert.equal(
    ...
    createHmac('sha1', 'test-secret').update(credentials.username).digest('base64')
  )
})
...
test('community fallbacks remain when self-hosted TURN is disabled', () => {
  const servers = createIceServers({})
  assert.deepEqual(servers.slice(0, PUBLIC_STUN_SERVERS.length), PUBLIC_STUN_SERVERS)
  assert.deepEqual(servers.slice(PUBLIC_STUN_SERVERS.length), COMMUNITY_TURN_SERVERS)
})
