...
import { resolveChannelRoomId } from "../app/shared/media/channel-room.ts";
...
describe("voice join resolves the room before connecting", () => {
  ...
  it("forwards connect options through the browser engine adapter", async () => {
    const source = await readFile(
      "app/composables/media/browserMediaEngine.ts",
      "utf8",
    );
    assert.match(
      source,
  ...
  it("passes the resolved room id to the media bootstrap", async () => {
    const source = await readFile(
      "app/shared/hybrid-media-session-lifecycle.ts",
      "utf8",
    );
    assert.match(
      source,
