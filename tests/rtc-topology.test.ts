...
} from "../app/shared/rtc-topology.ts";
import {
  directIceServers,
...
test("direct P2P configuration gathers STUN candidates without TURN allocations", () => {
  assert.deepEqual(
    directIceServers([
      {
...
test("peer connection attributes only remotely reported outbound loss to the local edge", async () => {
  const report = new Map([
    ...
    [
      "outbound",
      { id: "outbound", type: "outbound-rtp", kind: "video", packetsSent: 100 },
    ],
    [
      "remote-inbound",
      {
...
test("peer connection diagnostic report retains raw browser statistics and states", async () => {
  const report = new Map([
  ...
  const diagnostics = await collectPeerConnectionDiagnosticStats(
    {
      connectionState: "connected",
      iceConnectionState: "connected",
  ...
  assert.equal(diagnostics.kind, "p2p:peer-2");
  assert.deepEqual(diagnostics.pcStates, {
    connectionState: "connected",
    iceConnectionState: "connected",
    iceGatheringState: "complete",
    signalingState: "stable",
  });
  assert.deepEqual(diagnostics.stats, [...report.values()]);
});
