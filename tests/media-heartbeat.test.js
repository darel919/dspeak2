...
import {
  ...
  MEDIA_SIGNAL_HEARTBEAT_TIMEOUT_MS,
} from "../server/utils/media-heartbeat.js";
...
test("media signaling heartbeat tolerates short stalls and expires silent peers", () => {
  ...
  assert.equal(
    isMediaSignalHeartbeatExpired(
      startedAt,
      startedAt + MEDIA_SIGNAL_HEARTBEAT_TIMEOUT_MS - 1,
    ),
    false,
  );
  assert.equal(
    isMediaSignalHeartbeatExpired(
      startedAt,
      startedAt + MEDIA_SIGNAL_HEARTBEAT_TIMEOUT_MS,
    ),
    true,
  );
...
test("only complete non-negative integer heartbeat data refreshes signaling liveness", () => {
  ...
  assert.equal(
    isValidMediaSignalHeartbeat({ sequence: 1, topologyEpoch: 2 }),
    false,
  );
  assert.equal(
    isValidMediaSignalHeartbeat({
