...
import { acquireSharedRoom, isRoomUnused, releaseRoomReservation } from '../server/utils/room-lifecycle.js'
...
test('concurrent first joins share one room creation and reserve it from disposal', async () => {
  ...
  let createCount = 0
  let finishCreation
  const creationGate = new Promise(resolve => { finishCreation = resolve })
  const create = async () => {
    createCount += 1
    await creationGate
...
test('a failed room creation is removed so a later join can retry', async () => {
  const rooms = new Map()
  const creations = new Map()
  await assert.rejects(acquireSharedRoom({
    rooms,
    creations,
    key: 'room-1',
