...
test('outbound audio bitrate uses RTP byte and timestamp deltas', () => {
  const first = collectOutboundAudioStats(new Map([['audio', {
    type: 'outbound-rtp',
    kind: 'audio',
    timestamp: 1000,
    bytesSent: 1000,
  ...
  const second = collectOutboundAudioStats(new Map([['audio', {
    type: 'outbound-rtp',
    kind: 'audio',
    timestamp: 2000,
    bytesSent: 17_000,
...
test('outbound audio sampling ignores non-audio RTP reports', () => {
  const result = collectOutboundAudioStats(new Map([['video', {
    type: 'outbound-rtp',
    kind: 'video',
    timestamp: 1000,
    bytesSent: 1000
