...
test("video frame rate is normalized to the nearest supported preset", () => {
  assert.equal(
    normalizeVideoSettings({ frameRate: 1 }).frameRate,
    VIDEO_FRAME_RATE_MIN,
  );
  assert.equal(
    normalizeVideoSettings({ frameRate: 120 }).frameRate,
    VIDEO_FRAME_RATE_MAX,
  );
  ...
  assert.equal(
    normalizeVideoSettings({ qualityPriority: "resolution" }).qualityPriority,
    "resolution",
  );
  assert.equal(
    normalizeVideoSettings({ qualityPriority: "unknown" }).qualityPriority,
    "framerate",
  );
});
...
test("resolution priority permits capture cadence down to 24 FPS", () => {
  ...
  assert.deepEqual(constraints.frameRate, {
    min: VIDEO_RESOLUTION_PRIORITY_FRAME_RATE_MIN,
    ideal: 60,
    max: 60,
  });
});
...
test("P2P codec negotiation prefers H264 while retaining browser auxiliary codecs", () => {
  ...
  assert.deepEqual(
    sortP2pVideoCodecPreferences(codecs).map((codec) => codec.mimeType),
    ["video/H264", "video/VP9", "video/VP8", "video/rtx"],
  );
});

test("video production bitrate remains bounded for low and very large sources", () => {
  assert.equal(
    buildVideoProduceOptions({ width: 320, height: 240, frameRate: 25 })
      .encodings[0].maxBitrate,
    2_000_000,
  );
  const large = buildVideoProduceOptions({
    width: 7680,
    height: 4320,
    frameRate: 60,
    screen: true,
...
test("video adaptation accepts normal 60 FPS encoder variation as healthy", () => {
  ...
  for (let sample = 0; sample < 5; sample++)
    state = updateVideoAdaptationState(state, 56, 60);
...
test("video adaptation restores resolution gradually after sustained healthy FPS", () => {
  ...
  for (let sample = 0; sample < 5; sample++)
    state = updateVideoAdaptationState(state, 59, 60);
...
test("frame processing time uses the latest encoded-frame interval", () => {
  ...
  assert.equal(
    calculateFrameTimeMs(2, 125, { totalEncodeTime: 1.5, framesEncoded: 100 }),
    20,
  );
  assert.equal(
    calculateFrameTimeMs(2, 100, { totalEncodeTime: 1.5, framesEncoded: 100 }),
    null,
  );
});
...
test("encoded FPS uses RTP statistics timestamps so delayed background timers stay accurate", () => {
  assert.equal(
    calculateEncodedFps(220, 5000, { framesEncoded: 100, timestamp: 1000 }),
    30,
  );
  assert.equal(
    calculateEncodedFps(100, 5000, { framesEncoded: 100, timestamp: 1000 }),
    0,
  );
...
test("received bitrate uses byte and RTC timestamp deltas", () => {
  assert.equal(
    calculateBitrateKbps(1_100_000, 2000, { bytes: 100_000, timestamp: 1000 }),
    8000,
  );
  assert.equal(calculateBitrateKbps(100, 1000, null), null);
  assert.equal(
    calculateBitrateKbps(50, 2000, { bytes: 100, timestamp: 1000 }),
    null,
  );
});
...
test("codec implementation reports only evidence available from the browser", () => {
  ...
  assert.equal(
    classifyCodecImplementation(null).label,
...
test("power-efficient codec selection uses device evidence for the exact video configuration", async () => {
  ...
  const queried = [];
  const codec = await selectPowerEfficientVideoCodec(
    [vp8, h264],
    { width: 2940, height: 1912, framerate: 60 },
    {
      async encodingInfo(configuration) {
        queried.push(configuration);
        return {
          supported: true,
          powerEfficient: configuration.video.contentType === "video/VP8",
        };
      },
    },
  );
  ...
  const ranked = await rankVideoCodecsByHardwarePreference(
    ...
    {},
    {
      async encodingInfo({ video }) {
        return {
          supported: true,
          powerEfficient: video.contentType === "video/VP8",
        };
      },
    },
  );
...
test("codec capability inspection retains the exact browser report and query failures", async () => {
  ...
  const reports = await inspectVideoCodecCapabilities(
    ...
    {
      async encodingInfo({ video }) {
        if (video.contentType.startsWith("video/VP9"))
          throw new Error("VP9 query rejected");
...
test("H264 capability inspection probes common WebRTC profiles independently", async () => {
    ...
    {},
    {
      async encodingInfo({ video }) {
        ...
        return {
          supported: true,
          smooth: true,
          powerEfficient: video.contentType.includes("profile-level-id=42e01f"),
        };
      },
    },
  );
  ...
  assert.deepEqual(
    reports.map((report) => report.profileLevelId),
    ["42e01f", "42001f", "4d001f", "42e02a"],
  );
