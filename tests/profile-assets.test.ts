...
import {
  profileAssetUrl,
  profileInitials,
} from "../app/shared/profile-assets.ts";
import { avatarFileName, sameOriginAvatarPath } from "../shared/avatar-path.ts";
...
test("avatar proxy serves protected assets via presigned R2 URLs", () => {
  const source = readFileSync(
    new URL("../server/utils/dspeak-assets-api.ts", import.meta.url),
    "utf8",
  );
...
test("animated GIF profile pictures are accepted and preserved end to end", () => {
  const api = readFileSync(
    new URL("../server/utils/dspeak-profile-api.ts", import.meta.url),
    "utf8",
  );
