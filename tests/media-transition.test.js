...
test('topology keys distinguish source revisions and reject stale epochs', () => {
  assert.notEqual(
    topologyEventKey({ epoch: 4, mode: 'switching', target: 'sfu', sourceRevision: 1 }),
    topologyEventKey({ epoch: 4, mode: 'switching', target: 'sfu', sourceRevision: 2 })
  )
...
test('client and server topology policies remain identical', () => {
  for (let count = 0; count <= 6; count++) assert.equal(serverMembershipTopology(count), membershipTopology(count))
  const peerIds = ['a', 'b']
  const complete = new Map([['a', new Set(['b'])], ['b', new Set(['a'])]])
  assert.equal(serverCompleteMesh(peerIds, complete), hasCompleteMesh(peerIds, complete))
  const readiness = new Map([['a', 3], ['b', 3]])
  assert.equal(serverClientsReady(peerIds, readiness, 3), allClientsReady(peerIds, readiness, 3))
})
