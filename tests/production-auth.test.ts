...
const applicationSources = await Promise.all(
  [
    "../app/stores/rooms.ts",
    "../app/stores/channels.ts",
    "../app/stores/chat.ts",
    "../app/stores/notifications.ts",
    "../app/stores/identity.ts",
...
const supabaseAuth = await readFile(
  new URL("../server/auth/supabase.ts", import.meta.url),
  "utf8",
);
const oauthCallback = await readFile(
  new URL("../server/routes/api/auth/callback.get.ts", import.meta.url),
  "utf8",
);
const oauthCallbackSession = await readFile(
  new URL(
    "../server/routes/api/auth/callback-session.post.ts",
    import.meta.url,
  ),
  "utf8",
);
...
const profilesRepository = await readFile(
  new URL("../server/db/repositories/profiles.ts", import.meta.url),
  "utf8",
);
const randomUsername = await readFile(
  new URL("../server/auth/random-username.ts", import.meta.url),
  "utf8",
);
const oauthProfile = await readFile(
  new URL("../server/auth/oauth-profile.ts", import.meta.url),
  "utf8",
);
...
test("OAuth code exchange has a bounded provider wait", async () => {
  const exchange = await readFile(
    new URL("../server/auth/oauth-exchange.ts", import.meta.url),
    "utf8",
  );
