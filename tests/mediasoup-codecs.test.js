...
test('H264 router capabilities offer hardware-efficient profiles with a compatibility fallback', () => {
  ...
  assert.deepEqual(h264.map(codec => codec.parameters['profile-level-id']), ['42001f', '4d001f', '42e01f'])
  assert.ok(h264.every(codec => codec.parameters['level-asymmetry-allowed'] === 1))
  assert.ok(h264.every(codec => codec.parameters['packetization-mode'] === 1))
})
