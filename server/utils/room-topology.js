...
export function ensureRoomTopology(topology) {
  ...
  if (!(topology.transitionReadiness instanceof Map)) topology.transitionReadiness = new Map()
  return topology
}
...
export class RoomTopologyCoordinator {
  ...
  reconcile(room, reason) {
    ...
    this.beginTransition(room, 'sfu', room.topology.mode === 'p2p'
  ...
  beginTransition(room, target, reason) {
    ...
    room.topology.transitionTimer = this.setTimer(() => {
      ...
      if (target === 'sfu') return this.beginTransition(room, 'sfu', 'retrying-sfu-preparation')
  ...
  clientReady(room, peerId, { epoch, target, sourceRevision }) {
    ...
    if (Number(epoch) !== room.topology.epoch || String(target || '') !== room.topology.target) return false
    if (Number(sourceRevision) !== room.topology.sourceRevision || !room.sessions.has(peerId)) return false
  ...
  clientFailed(room, { epoch, target, sourceRevision, reason }) {
    ...
    const detail = String(reason || 'preparation-failed').toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').slice(0, 80)
    room.topology.transitionTimer = this.setTimer(() => {
      if (room.topology.epoch === Number(epoch) && room.topology.mode === 'switching') {
  ...
  p2pFailed(room, reason, epoch) {
    const directIsActive = room.topology.mode === 'p2p' || room.topology.mode === 'probing' || (room.topology.mode === 'switching' && room.topology.target === 'p2p')
    if (!directIsActive || Number(epoch) !== room.topology.epoch || room.sessions.size < 2) return false
    const detail = String(reason || 'unknown').toLowerCase().replace(/[^a-z0-9-]/g, '').slice(0, 48) || 'unknown'
    this.fallbackToSfu(room, `direct-path-unavailable-${detail}`)
    return true
  }

  sfuFailed(room, reason, epoch) {
    if (room.topology.mode !== 'sfu' || Number(epoch) !== room.topology.epoch || !room.sessions.size) return false
    const detail = String(reason || 'unknown').toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').slice(0, 64)
