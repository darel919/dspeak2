...
export default defineWebSocketHandler({
  async open(peer) {
    try {
      ...
      if (!channelId || !userId) return peer.close(1008, 'User ID and channel ID are required')
      ...
      sessions.set(peer.id, { userId, channelId, isMedia: Boolean(channel.isMedia) })
      addChannelSubscriber(channelId, peer)

      if (!channel.isMedia) {
        const current = (channel.inRoom || []).map(String)
        const inRoom = current.includes(String(userId)) ? current : [...current, userId]
        if (inRoom.length !== current.length) {
          await pb.collection('dspeak_rooms_channels').update(channelId, { inRoom })
        }
        send(peer, 'connected', { channelId, userId, message: 'Connected to channel' })
        broadcastToChannel(channelId, { type: 'user_joined', data: { userId, channelId } })
  ...
  async close(peer) {
    ...
    try {
      if (!session.isMedia) {
        const pb = await usePocketBaseAdmin()
        const channel = await pb.collection('dspeak_rooms_channels').getOne(session.channelId)
        const inRoom = (channel.inRoom || []).map(String).filter(id => id !== String(session.userId))
        await pb.collection('dspeak_rooms_channels').update(session.channelId, { inRoom })
        broadcastToChannel(session.channelId, { type: 'user_left', data: session })
        broadcastToChannel(session.channelId, { type: 'currentlyInChannel', inRoom })
      }
    } catch (error) {
