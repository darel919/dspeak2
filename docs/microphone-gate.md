...
- **Automatic:** Measures the local noise floor and chooses an opening threshold.
  ...
  stays 20 dB above the estimated floor, bounded between -52 dBFS and -28 dBFS,
  ...
  initial floor only after remaining nearly level for three seconds. Call VAD
  requires sustained qualifying samples before announcing speech, preventing a
  brief click or background-noise spike from triggering speech priority.
