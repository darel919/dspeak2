...
`desktop-build.yml` currently builds the dSpeak Tauri app for macOS (arm64),
Windows (x64), and Windows (arm64), then uploads the installers as run artifacts
and, on version tags, publishes them to a GitHub Release.
...
The native-media workflow targets macOS arm64, Windows x64, and Windows arm64.
It uses the same `desktop/scripts/provision-native-media.sh` provisioner used by
`dev:desktop`. macOS uses the pinned libwebrtc archive published by
libmediasoupclient because the self-hosted runner has limited disk space. Both
Windows targets build libwebrtc from the pinned WebRTC sources.
...
| Windows arm64 | `windows-11-arm`        | `dspeak-windows-arm64` | `.msi`, NSIS `.exe` |
...
- `native-media.yml` must have produced a successful bundle for the platform
  ...
  (macOS arm64, Windows x64, or Windows arm64).
