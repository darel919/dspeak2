export async function applyRtpSenderSettings(sender, options = {}) {
  ...
  for (const key of ['maxBitrate', 'maxFramerate', 'priority', 'networkPriority', 'scaleResolutionDownBy']) {
    if (requested[key] != null) encoding[key] = requested[key]
  }
  if (options.dtx != null) encoding.dtx = options.dtx
  if (options.degradationPreference) parameters.degradationPreference = options.degradationPreference
