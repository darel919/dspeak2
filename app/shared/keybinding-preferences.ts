...
export function normalizeCustomKeybindings<T>(value: T) {
  ...
  for (const [id, keys] of Object.entries(value)) {
    ...
    const validKeys = keys.filter(
      (key): key is string =>
        isExternalString(key) && key.length > 0 && key.length <= 80,
    );
...
export function saveCustomKeybindings<T>(value: T): void {
  if (!import.meta.client) return;
