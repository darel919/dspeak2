...
export function classifyTopology({ mode, participantCount, candidatePair, healthy = true }) {
...
export function buildTopologyGraph(snapshot = {}) {
  ...
  const showP2p = classification.mode === 'p2p-direct' || classification.mode === 'p2p-mesh' || (switching && (snapshot.currentMode === 'p2p' || snapshot.target === 'p2p'))
  const showSfu = classification.mode === 'sfu' || (switching && (snapshot.currentMode === 'sfu' || snapshot.target === 'sfu'))
  const sfuAddressFamily = addressFamily(snapshot.candidatePair?.remote?.address)
    ...
    for (let left = 0; left < nodes.length; left += 1) {
      for (let right = left + 1; right < nodes.length; right += 1) {
        ...
        edges.push({
          from: nodes[left].id,
          to: nodes[right].id,
          state: switching && snapshot.target === 'p2p' ? 'probing' : detail.state || 'active',
          transport: 'p2p',
          ...detail
        })
      }
  ...
  if (showSfu) {
    nodes.push({ id: 'sfu', role: 'sfu', health: snapshot.healthy === false ? 'degraded' : 'healthy' })
