<template>
  <div class="rtc-chart" :aria-label="`${label} history`" role="img">
    ...
    <div class="rtc-chart-axis">
      <span>{{ startLabel }}</span
      ><span>60 second window</span><span>{{ endLabel }}</span>
    </div>
  </div>
</template>
...
const usableSamples = computed(() =>
  props.samples.filter((sample) => Number.isFinite(sample.value)),
);
const endTimestamp = computed(
  () => props.samples.at(-1)?.timestamp || Date.now(),
);
const startTimestamp = computed(
  () => endTimestamp.value - props.windowSeconds * 1000,
);
const axisMax = computed(() => {
  const observed = Math.max(
    0,
    ...usableSamples.value.map((sample) => sample.value),
  );
...
const points = computed(() => {
  ...
  return usableSamples.value
    .map((sample) => {
      const x = Math.max(
        0,
        Math.min(
          420,
          ((sample.timestamp - startTimestamp.value) * 420) /
            (props.windowSeconds * 1000),
        ),
      );
...
const areaPath = computed(() =>
  points.value
    ? `M 0 96 L ${points.value.replaceAll(" ", " L ")} L 420 96 Z`
    : "",
);
...
.rtc-chart-axis span:nth-child(2) {
  text-align: center;
}
.rtc-chart-empty {
  position: absolute;
