    ...
    <div v-if="voiceStore.connected && voiceStore.currentChannelId === props.channel.id" class="flex-1 flex min-h-0 flex-col overflow-hidden">
      ...
      <div :class="videoFeeds.length ? 'shrink-0 border-t border-base-300 bg-base-200/70 px-4 py-3' : 'flex-1 p-6'">
          ...
          >
            ...
            >
              ...
              <div v-if="volumeMenuUser && volumeMenuUser.id === user.id" class="absolute top-2 right-2 bg-base-200 border border-base-300 rounded-lg shadow-lg p-3 z-50 w-48">
                ...
                <input type="range" min="0" max="1" step="0.01" :value="voiceStore.getTrackVolume(user.id, 'audio')" @input="onTrackVolumeChange(user.id, 'audio', $event)" class="w-full" />
                <div class="flex justify-between text-xs mt-1">
                  <span>0%</span>
                  <span>100%</span>
                </div>
                <template v-if="hasAudioSource(user.id, 'screen-audio')">
                  <div class="mt-3 text-xs font-semibold mb-1">Screen share</div>
                  <input type="range" min="0" max="1" step="0.01" :value="voiceStore.getTrackVolume(user.id, 'screen-audio')" @input="onTrackVolumeChange(user.id, 'screen-audio', $event)" class="w-full" />
                  <div class="flex justify-between text-xs mt-1">
                    <span>0%</span>
                    <span>100%</span>
                  </div>
        ...
        <div v-else class="h-full flex flex-col items-center justify-center">
          <Icon name="lucide:users" class="w-24 h-24 text-base-content/40 mb-6" />
    ...
    <div v-if="voiceStore.connected && voiceStore.currentChannelId === props.channel.id" class="border-t border-base-300 bg-base-300 p-4">
      <div class="flex items-center justify-center gap-4">
        ...
        <div class="flex flex-col items-center">
          ...
          >
            <Icon name="lucide:mic" v-if="!voiceStore.micMuted" class="w-6 h-6 text-current" />
        ...
        <div class="flex flex-col items-center">
          ...
          >
            <Icon name="lucide:volume-2" v-if="!voiceStore.deafened" class="w-6 h-6" />
...
function hasAudioSource(userId, source) {
  return Array.from(voiceStore.sfuComposable?.remoteAudioFeeds || [])
    .some(([, feed]) => String(feed.userId) === String(userId) && feed.source === source)
}
