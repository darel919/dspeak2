...
export const useNotificationsStore = defineStore('notifications', () => {
  ...
  async function getExistingSubscription() {
    ...
    try {
      ...
      if (existing?.endpoint && !existing.endpoint.includes(location.hostname)) {
  ...
  async function subscribe() {
    if (!checkPushSupport()) throw new Error('Push notifications are not supported')
    loading.value = true
    error.value = null
    try {
      ...
      const response = await fetch(`${config.public.apiPath}/chat/subscribe/global`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: userData.id },
        body: JSON.stringify({ subscription: pushSubscription.toJSON() }),
      })
      if (!response.ok) throw new Error(`Failed to register push subscription: ${response.status}`)
  ...
  async function unsubscribe() {
    ...
    try {
      ...
      if (!response.ok) console.warn('[NotificationsStore] Failed to unregister subscription:', response.status)
  ...
  async function updateSubscription() {
    if (!checkPushSupport() || Notification.permission !== 'granted') return null
    return subscribe()
  }
